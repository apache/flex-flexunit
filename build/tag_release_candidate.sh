...
if [ $# -ne 2 ]
then
    echo "Usage: tag_release_candidate flexunit_version ([0-99].[0-99].[0-999]) release_candidate ([0-100])"
fi
...
git tag -a apache-flexunit-${FLEXUNIT_VERSION}RC${RELEASE_CANDIDATE} -m \'"Apache FlexUnit ${FLEXUNIT_VERSION} RC${RELEASE_CANDIDATE}"\'
git push --tags
